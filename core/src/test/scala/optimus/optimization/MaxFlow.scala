...
trait MaxFlow extends AnyFunSpec with Matchers {
  ...
  describe("Max flow problem") {
    ...
    val x = Array.tabulate(lines.size, columns.size)((l, c) => MPFloatVar(s"x($l,$c)", 0, capacities(l)(c)))

    for (l <- 1 until lines.size) add(sum(columns)(c => x(l)(c)) - sum(lines)(c => x(c)(l)) := 0)
    ...
    it("all variables should have a value") {
      for (l <- lines; c <- columns) x(l)(c).value.isDefined shouldBe true
    }
    ...
    // Print solution
    for (l <- lines) {
      for (c <- columns) print(s"${x(l)(c).value.get} ")
      println()
    }
