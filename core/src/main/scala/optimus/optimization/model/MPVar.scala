...
class MPVar private[optimization] (
  ...
  def isUnbounded: Boolean = lowerBound == INFINITE && upperBound == INFINITE
...
object MPFloatVar {
   ...
   */
  def apply()(implicit model: MPModel) = new MPFloatVar(INFINITE, INFINITE, ANONYMOUS)
   ...
   */
  def apply(symbol: String)(implicit model: MPModel) = new MPFloatVar(INFINITE, INFINITE, symbol)
   ...
   */
  def positive()(implicit model: MPModel) = new MPFloatVar(0, INFINITE, ANONYMOUS)
   ...
   */
  def positive(symbol: String)(implicit model: MPModel) = new MPFloatVar(0, INFINITE, symbol)
   ...
   * @return a upper unbounded anonymous float variable
   */
  def negative()(implicit model: MPModel) = new MPFloatVar(INFINITE, 0, ANONYMOUS)
   ...
   * @return a upper unbounded float variable
   */
  def negative(symbol: String)(implicit model: MPModel) = new MPFloatVar(INFINITE, 0, symbol)
...
class MPIntVar private[optimization] (
  lowerBound: Int,
  upperBound: Int,
  override val symbol: String
)(implicit
  model: MPModel)
    extends MPVar(lowerBound, upperBound, symbol)(model) {
  override def isInteger: Boolean = true
}

class MPBinaryVar private[optimization] (override val symbol: String)(implicit model: MPModel)
    extends MPIntVar(0, 1, symbol)(model) {
...
object MPBinaryVar {
   ...
   */
  def apply()(implicit model: MPModel): MPBinaryVar = new MPBinaryVar(ANONYMOUS)
   ...
   */
  def apply(symbol: String)(implicit model: MPModel): MPBinaryVar = new MPBinaryVar(symbol)
}
